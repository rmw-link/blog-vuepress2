...
main
  ...
  &.spin
    background url('/spin.svg') 50% 50% / 19rem no-repeat
    height 19rem
    width @height
...
import {nextTick,shallowRef,reactive,onMounted} from 'vue'
import $on from './on'
import X from "./x.vue"
...
setup : =>
  eye = shallowRef(null)
  spin = shallowRef(true)

  onMounted =>
    router = useRouter()
    {pathname} = location
    if router.resolve(pathname).matched[0].name != "404" and pathname.replace('.html','') != "/404"
      return
    ...
    $on doc,{
      mousemove :(event) =>
        if eye.value
          ...
          eye.value.style = "transform:translate(#{xAxis}%,#{yAxis}%)"
        return
