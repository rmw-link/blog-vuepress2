# electron 完美拖拽 : 比 electron-drag 更优雅的实现

产品经理要求 electron window 下的工具条屏蔽鼠标右键。

window 下 electron 的原生拖拽(`app-region:drag`)会吃掉点击事件，没法做右键屏蔽。必须自己手工实现拖拽才行。

如果使用 mousemove 事件实现需求 ，在鼠标快速移动的时候，会超出窗体范围，导致窗体不再跟随鼠标移动。

我基于 pointer event 实现了这个需求，用 [setPointerCapture](https://developer.mozilla.org/zh-CN/docs/Web/API/Element/setPointerCapture) 可以捕获 BrowserWindow 之外的鼠标位置，效果很完美。

此方案比 electron-drag 的方案更优 : 它有平台相关的二进制依赖，升级打包都会变得麻烦。

我发现网上还没有人提过这种方案，所以写博客记录并分享一下。

实现方案的代码差异如下，仅够参考。核心代码是 `web/src/lib/drag.coffee` 和 `main/ipc/drag.coffee`。
