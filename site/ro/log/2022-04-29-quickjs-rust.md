...
Kraken [acceptă scrierea de extensii folosind `dart`](https://openkraken.com/guide/advanced/custom-js-api) .

Utilizarea [`flutter_rust_bridge`](https://github.com/fzyzcjy/flutter_rust_bridge) `rust` și `dart`.
...
Pe de altă parte, în timp ce comunitatea `rust` a [`rquickjs`](https://github.com/DelSkayn/rquickjs) astfel de apeluri către biblioteca `quickjs` din `rust`.  
Cu toate acestea, ele apelează `quickjs` în loc să integreze `quickjs`și nu pot fi folosite pentru a face magie pe `quickjs`.
...
În prezent, macroprocedura `#[js]` adaugă doar o constantă `fib_args_len`care identifică numărul de argumente ale funcției.
...
Puteți adăuga aceste funcții în funcție de necesități, consultați funcțiile din [qjs_sys](https://docs.rs/qjs-sys/0.1.2/qjs_sys/) începând cu `JS_To`.
...
Doar patru tipuri sunt definite pentru conversia din `None`, `()`, `i64`și CString în `js`. Puteți adăuga câte tipuri doriți.

Mai multe tipuri de date pot fi declarate în funcțiile din [qjs_sys](https://docs.rs/qjs-sys/0.1.2/qjs_sys/) începând cu `JS_New`.

## Mediu de dezvoltare

Eu dezvolt pe un laptop Apple, rust folosește 1.62.0-nightly.

Mai întâi instalați [direnv](https://direnv.net), mergeți la directorul și `direnv allow` pentru o vreme
...
* `./rust_macro`  
  ...
  În viitor, vedeți [wasmedge-quickjs](https://github.com/second-state/wasmedge-quickjs/blob/70efe8520dc65636cb81b7225e2a6dae47cfad49/src/quickjs_sys/mod.rs#L122) pentru un export automat de funcții rust în funcții js. [wasmedge-quickjs → JsFunctionTrampoline](https://github.com/second-state/wasmedge-quickjs/blob/70efe8520dc65636cb81b7225e2a6dae47cfad49/src/quickjs_sys/mod.rs#L122)
...
Cod din [quijine/main/quijine_core/src/ffi.rs](https://raw.githubusercontent.com/taskie/quijine/main/quijine_core/src/ffi.rs)
...
0. De la motorul `JS` la timpul de execuție `JS` [(sus)](https://github.com/doodlewind/blog/blob/master/source/_posts/js-engine-to-js-runtime-1.md) [(jos)](https://github.com/doodlewind/blog/blob/master/source/_posts/js-engine-to-js-runtime-2.md)
1. [Dezvoltarea unui modul nativ pentru `QuickJS` în C](https://github.com/quickjs-zh/QuickJS/wiki/%E4%BD%BF%E7%94%A8C%E8%AF%AD%E8%A8%80%E4%B8%BAQuickJS%E5%BC%80%E5%8F%91%E4%B8%80%E4%B8%AA%E5%8E%9F%E7%94%9F%E6%A8%A1%E5%9D%97)
2. [Utilizați Rust pentru a implementa JS API](https://wasmedge.org/book/en/dev/js/rust.html)
3. [Exemple QuickJS](https://github.com/Kozova1/quickjs-example)
4. [rugină-bindgen](https://rust-lang.github.io/rust-bindgen/)
5. [Cum să creați cod asincron pentru `QuickJS`](https://calbertts.medium.com/how-to-create-asynchronous-apis-for-quickjs-8aca5488bb2e)
...
10. [Capcana imperceptibilă - încuietori în Rust](https://mp.weixin.qq.com/s/BKto24ItwXbeHon_LaF_0w)

## Despre

Acest proiect face parte din proiectul de cod **rmw.link ( [rmw.link](//rmw.link)** ).
