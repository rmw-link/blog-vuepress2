# electron perfect drag and drop : elegantsem implementatsioon kui electron-drag

Tootejuht soovis, et elektronakna all olev tööriistariba blokeeriks hiire parema nupu.

Native drag and drop in electron under window ( `app-region:drag`) sööb kliki sündmuse ja ei ole võimalik teha parema kliki blokeerimist. Sa pead rakendama lohistamist käsitsi.

Kui kasutate nõude rakendamiseks mousemove-sündmust, siis kui hiir liigub kiiresti, läheb see akna ulatusest välja ja aken ei järgi enam hiirt.

Ma olen rakendanud selle nõude, mis põhineb osutiüritusel, kasutades [setPointerCapture'i](https://developer.mozilla.org/zh-CN/docs/Web/API/Element/setPointerCapture), et tabada hiire positsioon väljaspool BrowserWindow'd, ja see töötab suurepäraselt.

See lahendus on parem kui electron-drag lahendus: sellel on platvormi sõltuvad binaarsed sõltuvused ja selle uuendamine ja pakendamine võib olla tülikas.

Leidsin, et keegi pole seda lahendust veel veebis maininud, seega kirjutan sellest blogi ja jagan seda.

Koodierinevused rakenduste vahel on loetletud allpool ainult viitena. Põhikood on `web/src/lib/drag.coffee` ja `main/ipc/drag.coffee`.
