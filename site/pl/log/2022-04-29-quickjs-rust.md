...
[Kraken](https://openkraken.com) to wysokowydajny silnik renderujący strony internetowe oparty na witrynie `Flutter`, który wykorzystuje [quickjs](https://github.com/openkraken/kraken/tree/main/bridge/third_party/quickjs) jako silnik skryptowy.

Chciałem napisać kilka rozszerzeń do Krakena, korzystając z witryny `rust`.

Kraken [obsługuje pisanie rozszerzeń za pomocą strony `dart`](https://openkraken.com/guide/advanced/custom-js-api) .

Korzystanie z witryny [`flutter_rust_bridge`](https://github.com/fzyzcjy/flutter_rust_bridge) `rust` i `dart`.
...
Z drugiej strony, podczas gdy społeczność `rust` ma [`rquickjs`](https://github.com/DelSkayn/rquickjs) takie połączenia z biblioteką `quickjs` na stronie `rust`.  
...
Można dodać te funkcje w razie potrzeby, patrz funkcje w [qjs_sys](https://docs.rs/qjs-sys/0.1.2/qjs_sys/) zaczynające się od `JS_To`.
...
Zdefiniowano tylko cztery typy do konwersji z `None`, `()`, `i64`i CString na `js`. Można dodać dowolną ich liczbę.

Więcej typów danych można zadeklarować w funkcjach w [qjs_sys](https://docs.rs/qjs-sys/0.1.2/qjs_sys/) zaczynających się od `JS_New`.

## Środowisko programistyczne

Pracuję nad projektem na laptopie Apple, na którym używana jest wersja 1.62.0-nightly.

Najpierw zainstaluj [direnv](https://direnv.net), przejdź do katalogu i `direnv allow` przez chwilę
...
* `./rust_macro`  
  ...
  W przyszłości, zobacz [wasmedge-quickjs](https://github.com/second-state/wasmedge-quickjs/blob/70efe8520dc65636cb81b7225e2a6dae47cfad49/src/quickjs_sys/mod.rs#L122) dla automatycznego eksportu funkcji rust do funkcji js. [wasmedge-quickjs → JsFunctionTrampoline](https://github.com/second-state/wasmedge-quickjs/blob/70efe8520dc65636cb81b7225e2a6dae47cfad49/src/quickjs_sys/mod.rs#L122)
...
## Problemy rozwojowe : Memo

### `quickjs_ffi`

Kod z [quijine/main/quijine_core/src/ffi.rs](https://raw.githubusercontent.com/taskie/quijine/main/quijine_core/src/ffi.rs)
...
0. Od silnika `JS` do runtime `JS` [(góra)](https://github.com/doodlewind/blog/blob/master/source/_posts/js-engine-to-js-runtime-1.md) [(dół](https://github.com/doodlewind/blog/blob/master/source/_posts/js-engine-to-js-runtime-2.md) )
1. [Tworzenie natywnego modułu dla `QuickJS` w języku C](https://github.com/quickjs-zh/QuickJS/wiki/%E4%BD%BF%E7%94%A8C%E8%AF%AD%E8%A8%80%E4%B8%BAQuickJS%E5%BC%80%E5%8F%91%E4%B8%80%E4%B8%AA%E5%8E%9F%E7%94%9F%E6%A8%A1%E5%9D%97)
2. [Użyj języka Rust do implementacji interfejsu API JS](https://wasmedge.org/book/en/dev/js/rust.html)
3. [Przykłady QuickJS](https://github.com/Kozova1/quickjs-example)
4. [rust-bindgen](https://rust-lang.github.io/rust-bindgen/)
5. [Jak utworzyć kod asynchroniczny dla strony `QuickJS`](https://calbertts.medium.com/how-to-create-asynchronous-apis-for-quickjs-8aca5488bb2e)
...
9. [wasmedge-quickjs → połączenie](https://github.com/second-state/wasmedge-quickjs/blob/8a65582265ecdd3171380feebf56b3bb8c34d183/src/quickjs_sys/mod.rs#L756)
10. [Niezauważalna pułapka - zamki w Rdzy](https://mp.weixin.qq.com/s/BKto24ItwXbeHon_LaF_0w)

## O stronie

Ten projekt jest częścią projektu kodu **rmw.link ( [rmw.link](//rmw.link)** ).
