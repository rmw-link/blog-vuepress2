...
You can define the database in a module with `lazy_static` and then simply introduce it and use it, for example:
...
At the same time, mdbx [improves many of the shortcomings](https://github.com/erthink/libmdbx#improvements-beyond-lmdb) of lmdb, so Erigon (the next-generation ethereum client) recently switched from LMDB to MDBX [^erigon].
...
`get` comes out as `Ok(Some(Bin([6])))`and can be converted to `&[u8]`.
...
In addition, `Str` also implements [`std::fmt::Display`](https://doc.rust-lang.org/std/fmt/trait.Display.html) , `println!("{}",k)` will output a readable string.
...
##### `.dup(key)` Iterator that returns all values corresponding to a key
...
##### `.del_val(key,val)` Exact Match Delete
...
Start with `cargo add mdbx-speedy`in your own project, then you can quickly customize the type ( see [github.com/rmw-lib/mdbx-example/02](https://github.com/rmw-lib/mdbx-example/blob/master/02/src/main.rs) for demo code).
