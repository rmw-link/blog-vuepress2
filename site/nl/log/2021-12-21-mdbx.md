...
Momenteel ondersteunt het bestaande `rust` pakket van [mdbx-rs (mdbx-sys)](https://github.com/vorot93/mdbx-rs/issues/1) van `mdbx` [windows niet](https://github.com/vorot93/mdbx-rs/issues/1), dus nam ik de taak op me om een versie te maken met windows ondersteuning.
...
Als we eenvoudig een enkele regel gegevens willen lezen of schrijven, kunnen we de syntactische suiker van een macro gebruiken.
...
waarbij `key` en `val` de gegevenstypen voor respectievelijk sleutels en waarden definiëren.
...
[Unquoting](https://doc.rust-lang.org/std/ops/trait.Deref.html) `Str` zal een string teruggeven, vergelijkbaar met `let k:&str = &k;`.
...
Naast `Str` en `Bin` heeft de wrapper ook data-ondersteuning voor [usize, u128, u64, u32, u16, u8, isize, i128, i64, i32, i16, i8, f32, f64](https://docs.rs/mdbx/latest/src/mdbx/type.rs.html#48).

#### Databank vlaggen

U kunt de databasevlaggen zien die zijn toegevoegd aan de gegevens in [examples/02.rs](https://github.com/rmw-lib/mdbx/blob/master/examples/02.rs) op `Test4` `flag DUPSORT`
...
##### `.dup(key)` iterator die alle waarden teruggeeft die overeenkomen met een sleutel
...
Interval iteratie wordt niet ondersteund [`RangeFull`](https://doc.rust-lang.org/std/ops/struct.RangeFull.html) , d.w.z. het gebruik van `..`wordt niet ondersteund, gebruik in plaats daarvan de hierboven genoemde [traversal](#%E9%81%8D%E5%8E%86).

#### `.rev_range` Omgekeerde intervallen

Als je een geïnverteerd interval wilt krijgen dat kleiner is dan of gelijk aan een waarde, kun je dit doen
...
Aangepaste type-implementatie [`FromMdbx`](https://docs.rs/mdbx/latest/mdbx/type/trait.FromMdbx.html) en [`ToAsRef`](https://docs.rs/mdbx/latest/mdbx/type/trait.ToAsRef.html) kan dan worden opgeslagen op `mdbx`.
...
>   
> 6. MDBX ondersteunt "Exclusive open" mode - wij gebruiken dit voor databasemigraties om te voorkomen dat andere readers de database kunnen benaderen tijdens het migratieproces.
>